...
const FITNESS_ACTIVITIES = [
  ...
  { value: 'general', label: 'General Fitness', defaultUnit: 'min', defaultDaily: 30, defaultWeekly: 150 },
];
...
const HIGHLIGHTS = [
  { icon: Target, label: 'Flexible Targets' },
  { icon: Dumbbell, label: 'Fast Logging' },
  { icon: TrendingUp, label: 'Weekly Stats' },
...
export default function RegisterPage() {
  ...
  if (frequencyMode === 'daily') {
    ...
    calculationExplanation = `${dailyVal} ${targetUnit}/day × 7 = ${computedWeeklyTarget} ${targetUnit}/week`;
    ...
    calculationExplanation = `${computedWeeklyTarget} ${targetUnit}/week (~${approxDaily}/day)`;
  ...
  return (
    <div className="min-h-screen lg:h-screen lg:overflow-hidden bg-cream font-outfit flex flex-col lg:flex-row">
      ...
      <div className="hidden lg:flex lg:w-1/2 bg-violet border-r-2 border-gray-900 flex-col justify-between p-8 xl:p-12">
        <div>
          <div className="flex items-center gap-3 mb-8 xl:mb-12">
            ...
            <h1 className="text-2xl xl:text-3xl font-extrabold text-white">Momentum</h1>
          </div>

          <div className="max-w-md">
            <h2 className="text-3xl xl:text-5xl font-black text-white leading-tight mb-4 xl:mb-6">
              Start your <br />fitness journey
            </h2>
            <p className="text-sm xl:text-base text-white/80 font-medium leading-relaxed">
              Set goals on your terms. Daily minutes, weekly distance, or workout counts are automatically converted into your weekly target.
            </p>
          </div>
        </div>
        ...
        <div className="flex flex-wrap gap-2.5">
          {HIGHLIGHTS.map(({ icon: Icon, label }) => (
            <span
              key={label}
              className="inline-flex items-center gap-2 bg-white/20 backdrop-blur-sm px-3.5 py-1.5 rounded-full border-2 border-white/40 text-xs xl:text-sm font-semibold text-white shadow-brutal-sm"
            >
      ...
      <div className="flex-1 flex items-center justify-center p-4 sm:p-6 lg:p-6 xl:p-10 overflow-y-auto max-h-screen">
        <div className="w-full max-w-sm sm:max-w-md py-4 my-auto">
          {/* Mobile brand */}
          <div className="lg:hidden flex items-center gap-2.5 mb-5">
            <div className="w-9 h-9 bg-violet rounded-xl border-2 border-gray-900 shadow-brutal-sm flex items-center justify-center">
          ...
          <div className="mb-4">
            <h2 className="text-2xl sm:text-3xl font-black text-gray-900 leading-tight">Create your account</h2>
            <p className="text-xs sm:text-sm text-gray-500 font-medium mt-0.5">Set up your profile and configure your training target.</p>
          </div>
          ...
          <form onSubmit={handleSubmit} className="space-y-3">
            ...
            <div className="bg-sand/60 border-2 border-gray-900 rounded-2xl p-3.5 shadow-brutal space-y-3">
              <div className="flex items-center justify-between">
                <div className="flex items-center gap-1.5">
                  <Target className="w-4 h-4 text-gray-900" />
                  <span className="font-extrabold text-xs sm:text-sm text-gray-900">Custom Target Builder</span>
                </div>
                ...
                <button
                  ...
                  className={`px-2 py-0.5 rounded-lg border transition-all flex items-center gap-1 text-[11px] font-bold ${
                    showInfo
              ...
              {showInfo && (
                <div className="p-3 bg-white border-2 border-gray-900 rounded-xl text-[11px] space-y-1.5 text-gray-700 shadow-brutal-sm">
                  <p className="font-bold text-gray-900">How Momentum Targets Work:</p>
                  <p>
                    Momentum tracks your fitness on a weekly cycle. Enter your goal in whatever format is natural (Daily, Weekly, or Sessions).
                  </p>
                  <p className="text-gray-500 font-medium">
                    We automatically convert your entry into your weekly total target.
                  </p>
                </div>
              )}

              {/* Activity Focus + Cadence Row */}
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-2">
                <div>
                  ...
                  <select
                    ...
                    className="w-full px-2.5 py-1.5 bg-white border-2 border-gray-900 rounded-xl font-bold text-xs focus:outline-none"
                  >
              ...
              {frequencyMode === 'sessions' ? (
                <div className="grid grid-cols-2 gap-2">
                  <div>
                    ...
                    <input
                      ...
                      className="w-full px-2.5 py-1.5 bg-white border-2 border-gray-900 rounded-xl font-bold text-xs"
                    />
                  </div>
                  <div>
                    <label className="block text-[11px] font-semibold text-gray-700 mb-0.5">
                ...
                <div className="grid grid-cols-5 gap-2">
                  <div className="col-span-3">
                    <label className="block text-[11px] font-semibold text-gray-700 mb-0.5">
                      {frequencyMode === 'daily' ? 'Daily Target' : 'Weekly Target'}
                    </label>
                    <input
                      type="number"
                      min="1"
                  ...
                  <div className="col-span-2">
                    ...
                    <select
                      ...
                      className="w-full px-2 py-1.5 bg-white border-2 border-gray-900 rounded-xl font-bold text-xs"
                    >
                      {UNIT_OPTIONS.map((u) => (
              ...
              <div className="p-2.5 bg-white border-2 border-gray-900 rounded-xl flex items-center justify-between shadow-brutal-sm">
                <div>
                  <span className="text-[9px] font-extrabold uppercase tracking-wider text-gray-500 block">
                ...
                <div className="text-right">
                  <span className="text-xl font-black text-gray-900">{computedWeeklyTarget}</span>
                  <span className="text-[10px] font-bold text-gray-600 block">{frequencyMode === 'sessions' ? 'min' : targetUnit}</span>
                </div>
              </div>
            </div>
            ...
            <button
              type="submit"
              disabled={loading}
              className="w-full py-3 bg-gray-900 text-white border-2 border-gray-900 rounded-xl font-bold text-sm sm:text-base shadow-brutal-violet hover:bg-violet hover:text-white transition-all active:translate-x-0.5 active:translate-y-0.5 active:shadow-none flex items-center justify-center gap-2 disabled:opacity-50 mt-2"
            >
              {loading ? (
                'Creating account...'
              ) : (
          ...
          <p className="mt-4 text-center text-xs sm:text-sm text-gray-500 font-medium">
